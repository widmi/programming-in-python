...
# Increase a counter in a dictionary and skip if the key does not exist
a_list = [1, 2, 3, 4, 5, 1, 2, 3, 5, 2, 5, 5, 5]
count_2 = {2: 0, 5: 0}  # We only want to count values 2 and 5
for i in a_list:
    try:
        count_2[i] += 1
    except KeyError:
        pass
# Note: This is just an illustrative example, for better performance we would
# e.g. use the numpy module for this particular example.


# Make our add() function more robust
def add(a, b):
    try:
        c = a + b
    except TypeError:
        print("a or b are not numbers. Trying to convert to float...")
        c = float(a) + float(b)
    return c
...
# More information: https://docs.python.org/3.6/tutorial/errors.html
