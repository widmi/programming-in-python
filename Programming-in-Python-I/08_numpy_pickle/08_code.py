...
my_list = [1, 2.5, 3, 4]
my_array = np.array(my_list)  # -> default float (=64bit on 64bit installation)
...
# (Explanation:
#  It will first create a slice of the array and then access index 2 in that
#  sliced array, which corresponds to two_dim[2].)
...
two_dim = np.array([[1, 2, 3, 4],
                    ...
                    [1, 2, 3, 4]])
two_dim[1, 1:4] = 0
print(f"two_dim[1, 1:4] = 0\n->\n{two_dim}\n")

# If we want to broadcast sub-arrays we should be specific in which dimension
# to broadcast to avoid hard-to-find errors.
# Example: Insert the same sub-array [7, 8] in the second dimension at
# position [1:3] for all entries in the first dimension (=broadcast along first
# dimension):
# Here is our sub-array. We specify its shape explicitly as (1, 2) to indicate
# that we want to broadcast over the first dimension:
sub_array_to_broadcast = np.array([[7, 8]])  # Our sub-array
print(sub_array_to_broadcast.shape)
# Now we insert it at position [1:3] for all entries in the first dimension
print(two_dim[:, 1:3].shape)  # This slice will access the wanted elements
# -> we will broadcast the sub-array of shape (1, 2) to the array (3, 2),
#    resulting in the sub-array to be repeated 3 times along the first axis
two_dim[:, 1:3] = sub_array_to_broadcast  # -> will be broadcasted
print(two_dim)
...
# optionally while using compression

# Storing NumPy arrays using compression:
...
# If you are dealing with large arrays that can't be kept in memory at once,
# you can use the NumPy memmap to only read parts of an array that is stored in
# a file:
...
# provides additional functions to pickle and can handle more object types. It
# works for many Python objects but can lead to errors when trying to pickle
# Python classes or exotic objects (e.g. frame, generator, traceback).
# In doubt, always verify if your object type was saved correctly by loading
# the object in a different Python session/interpreter and checking it!
#
# Homepage and help: https://docs.python.org/3/library/pickle.html
# The pickle documentation also works for the dill module for the interface.
...
# compressed NumPy arrays (on disk or in RAM) and only uncompress the part you
# currently need. I recommend it as the go-to filetype to store large data
# since it is very versatile and easy to use.
# Note: Numpy memmap and savez_compressed/load is, in my experience, faster if
# you only want to load a full array from the disk into the RAM.
