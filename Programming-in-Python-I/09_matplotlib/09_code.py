...
# Keeping plots open until figure window is closed:
plt.show(block=True)
...
#
# Grayscale image data is by default visualized using a colormap (=mapping of
# value to color) that uses colors as well. This is done since it allows us to
# better perceive the different brightness values (e.g. blue and red are easier
# to distinguish than two different gray-values for humans).

# Let's create a grayscale numpy array. matplotlib will, if not explicitly
# specified, assume float values in range 0-1 and uint8 data in range 0-255.
grayscale = np.zeros((50, 50), dtype=np.uint8)  # 0 = lowest brightness
# Set every 3rd pixel to max. brightness (uint8 -> 2^8 values -> range 0-255)
grayscale[::3, ::3] = 255
fig, ax = plt.subplots()
ax.imshow(grayscale)
ax.set_xticks([], [])  # Remove xaxis ticks
...
# Simple animations
#
# This might fail depending on your active backends, ffmpeg installation,
# and versions.
#
