...
writer = SummaryWriter(log_dir="results/experiment_00")
...
torch.save(dsnn, "results/trained_dsnn.pt")

# Loading trainable parameters of a model (the module must already be defined)
dsnn = torch.load("results/trained_dsnn.pt")
...
# Define a tensorboard summary writer that writes to directory `log_dir`
writer = SummaryWriter(log_dir="results/experiment_01")
...
# In the next unit we will see an example of how a typical ML repo looks like.
