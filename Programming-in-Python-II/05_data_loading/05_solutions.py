...
# Create a class 'ImageSet' inheriting the super class 'Dataset' from 'torch.utils.data'
# to load all color images from a directory. Images in the directory is then normalized
# each chanel separately to zero mean and standardized variation, using the following norm_mean, norm_std
# norm_mean = torch.tensor([0.485, 0.456, 0.406], dtype=torch.float32, device=device)
# norm_std = torch.tensor([0.229, 0.224, 0.225], dtype=torch.float32, device=device)
# Finally, using class 'DataLoader' from 'torch.utils.data import DataLoader'
# to split the image set into mini batches and print them out
# Hint: create for your own an image directory with small number of color images of small size e.g. (6x10)
# to easier to have a look on data.
...
image_set = ImageSet(image_directory)
image_loader = DataLoader(image_set, shuffle=True, batch_size=2, num_workers=0)

# see how the image data looks like
